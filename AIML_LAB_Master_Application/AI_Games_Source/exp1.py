...
def toggle_wall(event):
    ...
    if (x, y) in walls:
        walls.remove((x, y))
        canvas.itemconfig(cells[x][y], fill="green")
    else:
        walls.add((x, y))
        canvas.itemconfig(cells[x][y], fill="red")
...
for i in range(rows):
    row = []
    for j in range(columns):
        cell = canvas.create_rectangle(j * cell_size, i * cell_size, (j + 1) * cell_size, (i + 1) * cell_size, fill="green", outline="red")
        row.append(cell)
    cells.append(row)
